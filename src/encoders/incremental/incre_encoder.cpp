...
void IncreEncoder::encode_symmetry_break_first_node()
{
    ...
    for (int i = data->upper_bound; i > data->upper_bound - (data->upper_bound / 2); i--)
    {
...
void IncreEncoder::encode_symmetry_break_highest_degree_node()
{
    ...
    for (int i = max_node_id * data->upper_bound; i > (max_node_id * data->upper_bound) - (data->upper_bound / 2); i--)
    {
        data->cc->add_clause({-1 * int(i)});
    }
}
...
void IncreEncoder::encode_symmetry_break_lowest_degree_node()
{
    ...
    for (int i = min_node_id * data->upper_bound; i > (min_node_id * data->upper_bound) - (data->upper_bound / 2); i--)
    {
        data->cc->add_clause({-1 * int(i)});
    }
}