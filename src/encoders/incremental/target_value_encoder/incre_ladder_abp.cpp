#include "incre_ladder_abp.h"

#include "assert.h"
#include <cmath>

int IncreLadderABP::get_target_value_aux_var(const IncreInstanceData &instance_data, std::vector<int> key, bool is_key_exist)
{
...
void IncreLadderABP::encode_target_value(const IncreInstanceData &instance_data)
{
    ...
    for (int vertex = 0; vertex < instance_data.global_data.g.n; vertex++)
    {
        ...
        {
            ladder_vars.push_back(vertex * instance_data.global_data.upper_bound + label + 1);
        }
...
void IncreLadderABP::encode_ladder(const IncreInstanceData &instance_data, const std::vector<int> ladder_vars, int width)
{
...
void IncreLadderABP::encode_window(const IncreInstanceData &instance_data, const std::vector<int> window_vars, bool is_first_window, bool is_last_window)
{
    if (instance_data.global_data.verbose)
    {
...
void IncreLadderABP::connect_windows(const IncreInstanceData &instance_data, const std::vector<int> first_window_vars, const std::vector<int> second_window_vars)
{
    if (instance_data.global_data.verbose)
    {
...
void IncreLadderABP::connect_ladder(const IncreInstanceData &instance_data, const std::vector<int> first_ladder_vars, const std::vector<int> second_ladder_vars, int width)
{
    if (instance_data.global_data.verbose)
    {
...
void IncreLadderABP::encode_labels(const IncreInstanceData &instance_data)
{
    for (int vertex = 0; vertex < instance_data.global_data.g.n; vertex++)
    {
        ...
        {
            int start = vertex * instance_data.global_data.upper_bound + window * instance_data.global_data.target_value + 1;
            int end = std::min(
                vertex * instance_data.global_data.upper_bound + (window + 1) * instance_data.global_data.target_value,
                vertex * instance_data.global_data.upper_bound + instance_data.global_data.upper_bound);
