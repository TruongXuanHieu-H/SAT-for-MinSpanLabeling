...
IteInstance::IteInstance(GlobalData &global_data, int label) : instance_data(global_data, label),
...
int IteInstance::encode_and_solve_problem()
{
    ...
    if (SAT_res == 0)
    {
        ...
        {
            if (!verifier.verify_target_value(instance_data.global_data.g, instance_data.global_data.target_value, label_assignment, instance_data.label))
            {
                std::cout << "s " << instance_data.get_signature() << " VERIFY FAILED: Target value not match.\n";
                exit(-1);
            }

            if (!verifier.verify_span(instance_data.global_data.g, instance_data.global_data.target_value, label_assignment, instance_data.label))
            {
                std::cout << "s " << instance_data.get_signature() << " VERIFY FAILED: Span not match.\n";
                exit(-1);
            }

            if (!verifier.verify_vertices(instance_data.global_data.g, instance_data.global_data.target_value, label_assignment, instance_data.label))
            {
                std::cout << "s " << instance_data.get_signature() << " VERIFY FAILED: Vertices constraints not satisfied.\n";
                exit(-1);
            }
        }
    }
